...
Чтобы начать пользоваться API, необходимо приобрести пакет QIWI Мастер.
...
В теле запроса передается JSON-объект [Payment](#payment_obj). Набор обязательных реквизитов платежа в объекте `fields`:
