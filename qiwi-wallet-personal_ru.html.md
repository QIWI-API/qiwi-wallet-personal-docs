...
* `<parameter>` - дополнительные параметры запроса (необязательные):
...
В JSON-теле запроса указываются параметры:
...
`https://edge.qiwi.com/payment-history/v1/persons/<wallet>/payments?<parameter>=<value>`
...
* `<wallet>` - номер кошелька, для которого получен токен доступа (с международным префиксом, но без `+`). **Обязательный параметр**
* `<parameter>` - дополнительные параметры запроса:
...
data[].txnId | Integer |ID транзакции в процессинге QIWI
...
provider.id|Integer|ID провайдера в процессинге QIWI,
...
`https://edge.qiwi.com/payment-history/v1/persons/<wallet>/payments/total?<parameter>=<value>`
...
`https://edge.qiwi.com/payment-history/v1/transactions/<transactionId>?type=<type>`
...
Успешный JSON-ответ содержит список счетов для фондирования платежей и их текущие балансы:
...
Успешный JSON-ответ содержит фрагмент с данными о комиссии:
...
Запрос предназначен для расчета суммы комиссии за платеж (с учетом всех тарифов) по заданному набору платежных реквизитов.
...
paymentMethod | Object| Объект, определяющий обработку платежа процессингом QIWI. Содержит следующие параметры:
...
Параметры в строке запроса `<parameter>=<value>`:
...
transaction|Object|Объект с данными о транзакции в процессинге QIWI. Параметры:
transaction.id|String|ID транзакции в процессинге QIWI
transaction.state|Object|Объект содержит текущее состояние транзакции в процессинге QIWI. Параметр:
...
Предварительная проверка мобильного номера выполняется POST-запросом на URL:
...
Запрос выполняет оплату услуги по идентификатору пользователя. Данный запрос применяется для провайдеров, использующих в реквизитах единственный пользовательский идентификатор, без проверки номера аккаунта.
