...
amount|Number|Текущий баланс данного счета
...
{"account":"380966447431","paymentMethod":{"type":"Account","accountId":"643"},"purchaseTotals":{"total":{"amount":"1001","currency":"643"}}}

## Сложные комиссии {#online-comm}

Запрос предназначен для расчета сложной комиссии на платеж (с учетом всех тарифов) по заданному набору платежных реквизитов.
...
user@server:~$ curl -X POST "https://edge.qiwi.com/sinap/providers/99/onlineCommission"
  ...
  --header "Content-Type: application/json"
  --header "Authorization: Bearer YUu2qw048gtdsvlk3iu"
  -d '{"account":"380995238345","paymentMethod":{"type":"Account","accountId":"643"},
...
{"account":"380995238345","paymentMethod":{"type":"Account","accountId":"643"},"purchaseTotals":{"total":{"amount":"10","currency":"643"}}}
~~~
...
source| String|Копия параметра `source` из исходного запроса.
transaction|Object|Объект с данными о транзакции в процессинге. Параметры:
id|String|ID транзакции в процессинге
state|Object|Объект содержит текущее состояние транзакции в процессинге. Параметр:
code | String| Текущий статус транзакции, только значение `Accepted` (платеж принят к проведению). Финальный результат транзакции можно узнать в [истории платежей](#payments).
