...
###### Последнее обновление: 2022-01-20 | [Предложить правки на GitHub](https://github.com/QIWI-API/qiwi-wallet-personal-docs/blob/master/_payment_ru.html.md)
...
* [Прочие банки](#banks).
* [Операторы мобильной связи](#mnp).
* [Другие провайдеры](#provider-search).
...
* 99 - Перевод на QIWI Wallet
* 99999 -  Перевод на QIWI Wallet по никнейму
...
В теле запроса передается JSON-объект [Payment](#payment_obj). Набор реквизитов платежа в объекте `fields`:
...
Запрос выполняет денежный перевод на карты платежных систем Visa, MasterCard или МИР. Предварительно можно [узнать идентификатор провайдера для перевода](#card_check) по номеру карты.
...
В теле запроса передается JSON-объект [Payment](#payment_obj).

Набор реквизитов платежа в поле `fields` зависит от ID провайдера.
...
Если вы не нашли нужный банк в списке, воспользуйтесь [поиском провайдера](#provider-search).
...
См. также раздел [Как найти идентификатор провайдера](#provider-search).
...
Параметр передается в теле запроса как `formdata`:
...
**searchPhrase** — строка ключевых слов, разделенных пробелами, для поиска провайдера.
